...
(function() {
	var a = function(b, c) {
			...
			if (D.lockRatio) {
				...
				if (F.getCustomData('isReady') == 'true')
					if (this.id == 'txtHeight') {
						if (C && C != '0')
							C = Math.round(F.$.width * (C / F.$.height));
						if (!isNaN(C))
							D.setValueOf('info', 'txtWidth', C);
					} else {
						if (C && C != '0')
							C = Math.round(F.$.height * (C / F.$.width));
						if (!isNaN(C))
							D.setValueOf('info', 'txtHeight', C);
					}
			}
			l(D);
		}, l = function(C) {
			if (!C.originalElement || !C.preview)
				return 1;
			C.commitContent(f, C.preview);
			return 0;
		};
		function m() {
			var C = arguments, D = this.getContentElement('advanced',
					'txtdlgGenStyle');
			D && D.commit.apply(D, C);
			this.foreach(function(E) {
				if (E.commit && E.id != 'txtdlgGenStyle')
					E.commit.apply(E, C);
			});
		}
		;
		...
		var p = function(C, D) {
			...
			if (E.getCustomData('isReady') == 'true') {
				if (D == 'check') {
					var G = C.getValueOf('info', 'txtWidth'), H = C.getValueOf(
							'info', 'txtHeight'), I = E.$.width * 1000
							/ E.$.height, J = G * 1000 / H;
					C.lockRatio = false;
					if (!G && !H)
						C.lockRatio = true;
					else if (!isNaN(I) && !isNaN(J))
						if (Math.round(I) == Math.round(J))
							C.lockRatio = true;
				} else if (D != undefined)
					C.lockRatio = D;
				else
				...
				C.lockRatio = false;
			if (C.lockRatio)
				F.removeClass('cke_btn_unlocked');
			else
				F.addClass('cke_btn_unlocked');
			var K = C._.editor.lang.image, L = K[C.lockRatio ? 'unlockRatio'
					: 'lockRatio'];
			...
			;
			var F = this.getDialog(), G = '', H = this.id == 'txtWidth' ? 'width'
					: 'height', I = D.getAttribute(H);
			if (I)
				G = E(I, G);
			G = E(D.getStyle(H), G);
			this.setValue(G);
		...
		return {
			title : c == 'image' ? b.lang.image.title
					: b.lang.image.titleButton,
			minWidth : 420,
			minHeight : 310,
			onShow : function() {
				var I = this;
				...
				I.firstLoad = true;
				I.addLink = false;
				var C = I.getParentEditor(), D = I.getParentEditor()
						.getSelection(), E = D.getSelectedElement(), F = E
						&& E.getAscendant('a');
				CKEDITOR.document.getById(y).setStyle('display', 'none');
				...
				if (F) {
					...
					if (G.count() == 1) {
						...
						if (H == 'img' || H == 'input') {
							...
							if (I.imageElement.getName() == 'img')
								I.imageEditMode = 'img';
							else if (I.imageElement.getName() == 'input')
								I.imageEditMode = 'input';
						}
					}
					if (c == 'image')
						I.setupContent(e, F);
				...
				if (E && E.getName() == 'img'
						&& !E.getAttribute('_cke_realelement') || E
						&& E.getName() == 'input'
						&& E.getAttribute('type') == 'image') {
				...
				if (I.imageEditMode) {
					...
					I.imageElement = C.document.createElement('img');
			...
			onOk : function() {
				...
				if (!D.imageEditMode) {
					if (D.addLink) {
						...
						var value=$("#cke_25_textInput").val();
						if(typeof value=='undefined'){
							value=$("#cke_24_textInput").val();
							if(typeof value=='undefined'){
								value=$("#cke_26_textInput").val();
							}
						}
						$("#beforePhoto").val(value);
									...
									commit : function(C, D) {
										...
										if (C == d) {
											if (E.getValue() || E.isChanged())
												D.setAttribute('alt', E
														.getValue());
										} else if (C == f)
											D.setAttribute('alt', E.getValue());
										else if (C == g)
																					...
																					validate : function() {
																						var C = this
																								...
																								.match(
																										i);
																						if (!C)
																							alert(b.lang.image.validateWidth);
																						return !!C;
																					},
																					setup : r,
																					commit : function(
																					...
																					validate : function() {
																						var C = this
																								...
																								.match(
																										i);
																						if (!C)
																							alert(b.lang.image.validateHeight);
																						return !!C;
																					},
																					setup : r,
																					commit : function(
																				...
																				D
																						.on(
																								...
																								function(
																										E) {
																									var J = this;
																									var F = p(J), G = J.originalElement, H = J
																											.getValueOf(
																													'info',
																													'txtWidth');
																									if (G
																	...
																	},
																	{
																		...
																		items : [
																				[
																						...
																						'' ],
																				[
																						b.lang.image.alignLeft,
																						'left' ],
																				[
																						b.lang.image.alignRight,
																						'right' ] ],
																		onChange : function() {
																			l(this
																					.getDialog());
								...
								},
								{
									...
									setup : function(C, D) {
										if (C == d) {
											...
											if (!E && D.$.style.cssText)
												E = D.$.style.cssText;
											this.setValue(E);
											var F = D.$.style.height, G = D.$.style.width, H = (F ? F
													: '').match(h), I = (G ? G
													: '').match(h);
	...
	CKEDITOR.dialog.add('code', function(b) {
		return a(b, 'code');
	});
